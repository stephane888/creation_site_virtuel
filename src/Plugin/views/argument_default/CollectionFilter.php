...
class CollectionFilter extends ArgumentDefaultPluginBase implements CacheableDependencyInterface {
  ...
  public function getArgument() {
    ...
    if (isset($entity)) {
      ...
      //Il faut trouver le moyen de determiner quel champ sélectionner de manière automatique
    ...
    foreach ($map as $entity_type => $fields) {
      foreach ($fields as $field_name => $field) {
        ...
        if ($config && $config->getSetting('target_type') == $target_type) {
          // Exécuter une requête d'entité pour trouver toutes les entités qui référencent l'ID cible
          return $field_name;
        }
      }
    }
